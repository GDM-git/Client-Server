...
public class S_11 implements Skill {
    public int get_priority() {return 19;}
    public void cast(Player user, Player enemy, int round){
        user.set_Mana_cost(round,3);
        enemy.set_Damage(round, enemy.get_Damage(round) + 6);
        enemy.set_Damage_over_time(round + 1,enemy.get_Damage_over_time(round + 1) + 2);
    }
    public String get_message(String Player_Name) {
        return (Player_Name + " выпускает огненную стрелу");
    }
}